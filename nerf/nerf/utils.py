...
class Trainer(object):
    ...
    def train_step(self, data):
        self.iter += 1/120000 # 50 min to reach 1
        rays_o = data['rays_o'] # [B, N, 3]
        rays_d = data['rays_d'] # [B, N, 3]
        var = data["vars"]
        mu = data["mus"]
        new_latents = reparameterize(mu, var)
        presampled_latents = data['latents']  # [B, latent_dim]
        if self.iter >= 1:
            self.iter = 1
        latents = self.iter * new_latents + (1-self.iter) * presampled_latents
        ...
        if 'images' not in data:
            ...
            outputs = self.model.render(rays_o, rays_d, latents, staged=False, bg_color=None, perturb=True, force_all_rays=True, **vars(self.opt))
        ...
        outputs = self.model.render(rays_o, rays_d, latents, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        rays_d = data['rays_d'] # [B, N, 3]
        H, W = data['H'], data['W']
        latents = data['latents']  # [B, latent_dim]
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            if self.model.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    self.model.update_extra_state(train_loader)
    ...
    def test_gui(self, pose, intrinsics, W, H, latents, bg_color=None, spp=1, downscale=1):
        ...
        if latents is None:
            ...
            # pass a dummy latent of 0
            batch_size = pose.shape[0]
            latents = torch.ones((batch_size, 1)).to(self.device)
        else:
            batch_size = rays['rays_o'].shape[1]
            # latents = latents.expand(batch_size, 128)
