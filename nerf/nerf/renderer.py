...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, latents, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, **kwargs):
        ...
        # one latent corresponds to one image and many rays
        '''
        try:
            latents = latents.contiguous().view(-1, self.latent_dim)
        except:
            print(latents.shape)
            raise
        '''
        ...
        if self.training:
            ...
            latents = latents.expand(xyzs.shape[0],self.latent_dim).contiguous().view(-1, self.latent_dim)
            sigmas, rgbs = self(xyzs, dirs, latents)
    ...
    @torch.no_grad()
    def update_extra_state(self, train_loader=None, decay=0.95, S=128):
        # call before each epoch to update extra states.

        # TODO unclear how to properly handle latents here... we want a pessimistic estimate since we don't
        #      want to skip ray marching
        batch_size = 1
        if train_loader is not None:
            index = np.random.randint(len(train_loader._data.paths))
            latents = train_loader._data.scene_latents[index]

        else:
            latents = torch.zeros((batch_size, 1)).to(self.density_grid.device)
                ...
                for ys in Y:
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            latents = latents.expand(cas_xyzs.shape[0], self.latent_dim)
                            sigmas = self.density(cas_xyzs, latents)['sigma'].reshape(-1).detach()
